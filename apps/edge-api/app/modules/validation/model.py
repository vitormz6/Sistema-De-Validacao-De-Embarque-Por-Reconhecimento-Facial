...
class LocalValidationLog(Base):
    """
    One row per boarding attempt (RF09). `id` doubles as the future
    `external_id` central-api's `/sync/push` expects — generated here,
    never reassigned, so a retried push is naturally idempotent on the
    central side once the sync-worker exists.

    `synced_at` stays NULL until the sync-worker confirms the push
    succeeded; `synced_at IS NULL` is this table's outbox queue (see
    edge-api.md for why a separate `local_sync_outbox` table was skipped).
    """
